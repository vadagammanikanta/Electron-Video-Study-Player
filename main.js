...
const CACHE_DIR = path.join(USER_DATA, 'video_cache');
...
ipcMain.handle('load-zips-from-folder', async () => {
  ...
  const oldMap = new Map((oldIdx.items||[]).map(it => [it.cachedPath || (it.zipName+'::'+it.entryName), it]));
  const items = [];
...
ipcMain.handle('load-index', async ()=>{
  ...
  for(const it of (idx.items||[])){
    try{ if(it.cachedPath && fs.existsSync(it.cachedPath)) items.push(it); }
    catch(e){}
  }
  idx.items = items;
